<template>
  <div class="home">
    ...
    <div v-if="tasks.length === 0">
      ...
      <p class="text-center blue--text">No Task</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    editTask() {
      db.collection("Tasks").doc(this.editTitle.id).update({
    ...
    cancelEditTask() {
      db.collection("Tasks").doc(this.beforeTask.id).update({
    ...
    deleteTaskSnackbar() {
      db.collection("Tasks").add(this.trashTask);
      this.trashTask = "";
    },
    addDateTask() {
      ...
      db.collection("Tasks").doc(this.changeID).update({
        date: time,
      });
